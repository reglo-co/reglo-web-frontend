...
import { getWorkspaceUrl } from '@/modules/workspaces/helpers'
...
export function WorkspaceSwitchUi({ teams }: { teams: WorkspaceMinimal[] }) {
  const { workspace } = useWorkspace()
  const { isMobile } = useSidebar()

  const activeTeam = teams.find((team) => team.slug === workspace)
      ...
      <SidebarMenuItem>
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <SidebarMenuButton
              size="lg"
              className="data-[state=open]:bg-sidebar-accent data-[state=open]:text-sidebar-accent-foreground h-12 gap-1"
            >
          ...
          >
            ...
            {teams.map((team, index) => (
              <DropdownMenuItem key={team.name} className="gap-2 p-2">
                <LinkWithLoading
                  href={getWorkspaceUrl(team.slug, '/')}
